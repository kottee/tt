...
class NovalnetOrderConfirmationDataProvider
{
    ...
    public function call(Twig $twig, PaymentRepositoryContract $paymentRepositoryContract, $arg)
    {
        ...
        $order = $arg[0];
        $barzhlentoken = (string)$sessionStorage->getPlugin()->getValue('cashtoken');
        $barzahlenurl = (string)$sessionStorage->getPlugin()->getValue('checkouturl');
        ...
        foreach($payments as $payment)
        {
            if($paymentHelper->getPaymentKeyByMop($payment->mopId))
            {
              ...
              return $twig->render('Novalnet::NovalnetOrderHistory', ['comments' => html_entity_decode($comment),'barzahlentoken' => html_entity_decode($barzhlentoken),'payment_type' => html_entity_decode($payment_type),'barzahlenurl' => html_entity_decode($barzahlenurl)]);
            }
        }
    }
}
