...
class PaymentService
{
        ...
        try {
            ...
            if(!$callbackfailure)
            {
                if((in_array($requestData['payment_id'], ['34','78']) && in_array($requestData['tid_status'], ['86','90','85'])))
                {
                    ...
                    {
                        $requestData['order_status'] = trim($this->config->get('Novalnet.novalnet_paypal_order_completion_status'));
                    }
	...
	$comments .= '</br>' . $this->paymentHelper->getTranslatedText('transaction_cancellation',$lang) . $responseText . '</br>';    
    ...
    public function getInvoicePrepaymentComments($requestData)
    {
        ...
        $comments .= PHP_EOL.'Payment Reference 1: TID '. $requestData['tid']. PHP_EOL . 'Payment Reference 2:'.' ' .('BNR-' . $requestData['product'] . '-' . $requestData['order_no']). PHP_EOL;
        $comments .= PHP_EOL;
	    }
        return $comments;
    }
